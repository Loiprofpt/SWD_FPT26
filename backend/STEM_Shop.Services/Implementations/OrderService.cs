...
namespace STEM_Shop.Services.Implementations
{
    public class OrderService : IOrderService
    {
            ...
            return await strategy.ExecuteAsync(async () =>
            {
                ...
                try
                {
                    ...
                    foreach (var item in cart.CartItems)
                    {
                        ...
                        {
                            throw new Exception($"Sản phẩm '{product.Name}' không đủ số lượng trong kho (Còn {product.StockQuantity}).");
                        }
                ...
                catch (Exception ex)
                {
                    ...
                    // Lấy lỗi gốc từ InnerException (nếu có) để debug dễ hơn
                    var msg = ex.InnerException?.Message ?? ex.Message;
                    return new ApiResponse<OrderResponse> { Success = false, Message = msg };
                }
            });
        }
        ...
        public async Task<ApiResponse<List<OrderResponse>>> GetMyOrdersAsync(int userId)
        {
            ...
            catch (Exception ex) { return new ApiResponse<List<OrderResponse>> { Success = false, Message = ex.InnerException?.Message ?? ex.Message }; }
        }
        ...
        public async Task<ApiResponse<OrderResponse>> UpdateMyOrderAsync(int userId, int orderId, UpdateMyOrderRequest request)
        {
            ...
            catch (Exception ex) { return new ApiResponse<OrderResponse> { Success = false, Message = ex.InnerException?.Message ?? ex.Message }; }
        }
        ...
        public async Task<ApiResponse<bool>> DeleteOrderAsync(int orderId)
        {
            try
            {
                ...
                // Trước khi xóa, nếu đơn chưa bị hủy thì hoàn kho
                if (order.Status != "Cancelled")
                {
                    foreach (var detail in order.OrderDetails)
                    {
                        if (detail.ProductId.HasValue)
                        {
                            ...
                            var ws = await _context.WarehouseStocks.FirstOrDefaultAsync(x => x.ProductId == detail.ProductId);
                            if (ws != null) ws.Quantity = (ws.Quantity ?? 0) + (detail.Quantity ?? 0);
                        }
                    }
                }
            ...
            catch (Exception ex) { return new ApiResponse<bool> { Success = false, Message = ex.InnerException?.Message ?? ex.Message }; }
        }
